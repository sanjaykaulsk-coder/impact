...
// docs/ASSUMPTIONS.md for why. All three signals below are computed entirely from data the backend
// already has, at call sites that already run inside a tenant/campaign context.
export type DeviceRiskSignal = 'DEVICE_TIME_MISMATCH' | 'MOCK_LOCATION_SUSPECTED' | 'DUPLICATE_MEDIA_SUSPECTED';
...
export const DEVICE_RISK_SIGNAL_TYPES: DeviceRiskSignal[] = [
  'DEVICE_TIME_MISMATCH',
  'MOCK_LOCATION_SUSPECTED',
  'DUPLICATE_MEDIA_SUSPECTED',
];
...
// active manipulation, so it restricts pending supervisor action. A perceptually-matched photo sits
// between the two — a real, worth-a-look signal (spec's own "old photograph" risk), but one with
// enough innocent explanations (two visits to a genuinely similar-looking stall) that it's flagged
// for review, not restricted outright.
export const DEFAULT_DEVICE_RISK_RULES: Record<DeviceRiskSignal, RiskLevel> = {
  ...
  DUPLICATE_MEDIA_SUSPECTED: 'L2_FLAGGED',
};

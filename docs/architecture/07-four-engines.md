...
- **Activity templates** are master configuration documents (milestones + default forms + KPI slots). Attaching one to a campaign *copies* it into campaign scope — campaign-level edits never touch the master library (spec §9.4). Building a fully custom activity for one campaign (custom form + custom workflow) already works today (Stages 3.1/3.2); *saving that combination back as a new, reusable master template* does not yet — confirmed as a real, narrow, unscheduled gap in `docs/ASSUMPTIONS.md` A-058, not an architecture change (the schema already anticipates it via `ActivityType.isCustom`).
...
- **Dashboards are configuration too:** a dashboard definition (widgets, KPIs, filters, drill-down path) rendered by one dashboard engine — Ops, Client Servicing, Director and Client dashboards are four configurations of the same engine, client dashboards additionally filtered to approved-only data per Impact-controlled access rules. Cross-location comparison, time-trend, and deviation/exception surfacing all fit this engine as already specified and are already sequenced (S5.2/S5.3) — confirmed against a founder question in `docs/ASSUMPTIONS.md` A-058. A *client self-service* KPI picker (client chooses which fields matter per campaign, distinct from Impact's own fixed drill-down hierarchy) remains the one piece still filed under the post-MVP backlog below, not inside S5.3 — a sequencing question for the founder, not a build question.

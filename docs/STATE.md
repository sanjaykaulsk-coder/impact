...
15. **Bug you found while testing S4.2, "Could not clear this device" on the Device Risk page — now actually fixed and proven, not just built (A-064 + A-065).** The first fix (a login-session timing issue) was real but didn't fully explain what you were hitting. This time it was reproduced directly — a real backend, a real browser, clicking the real Clear and Block buttons — and confirmed: the device really was being cleared/blocked correctly every single time, but the app was showing an error anyway because of how it read the server's (empty, but successful) response. Fixed, and this time verified against a live run before calling it done, not just a clean build. Please `git pull`, restart the backend and web admin, and try Clear again.

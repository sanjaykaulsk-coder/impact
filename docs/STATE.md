...
20. **Bug you found testing S4.3: "session timeout" on your phone right after updating the app — fixed, logged as A-067, awaiting your retest.** The phone app had the exact same kind of login-refresh bug as the Device Risk one (A-064) — the app's home screen loads several things at once, and if two of those happen to need a fresh login token at the same moment, one of them could wipe out the other's, signing you out. Adding the new Attendance card made this more likely to actually happen. Fixed the same way as before. This sandbox has no way to test a real phone, so — unlike the other fixes this session — **this one is not yet proven on real hardware**; please retest per the terminal steps below.

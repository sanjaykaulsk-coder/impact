...
- **The server side is fully built and tested for real** — given today's assigned stops for a field worker, the backend works out exactly which map tiles are needed, fetches them from OpenStreetMap the first time (and only the first time — after that, every future request for that same area reuses a saved copy, so we're not hammering a free public map service on every phone's every request), and hands them to the app. Tested with a real assignment in Patna, Bihar — downloaded one of the actual map images this produced and it's a genuine, correct street map of that exact area.
- **The phone-app side is written but genuinely untested this session** — a real map screen showing the field worker's stops (colour-coded: done / in progress / not yet visited) plus their live GPS position, using the downloaded map tiles so it keeps working with no signal. Added a "View Route Map" button to the phone app's home screen.
- **Important, more serious gap than usual, stated plainly**: this build machine does not have the Flutter phone-app toolkit installed at all this time — not just "no phone to test on" (the usual limitation), but the tool that even checks the phone-app code for typos and mistakes before it runs. Every previous phone-app change in this project at least got that basic check; this one didn't. The code was written as carefully as possible by hand, matching this app's own existing patterns exactly, but it has had **zero automatic checking of any kind** — you (or whoever next has a machine with the Flutter toolkit) are the very first checkpoint for whether it even compiles, let alone whether it looks and works right on a phone.

### Reviewing — Offline map corridors

1. **Backend** (safe to check right now, no phone needed): restart with `./scripts/bootstrap.sh` if needed. No new setup required — this uses your existing database and file storage.
2. **Phone app**: this needs a machine with Flutter installed (Android Studio or `flutter run`, same as always) — pull this branch there first. Log in as Rahul Kumar (`9000000009`) with a real assignment for today that has a location (see "How to see it yourself" below for creating a test assignment if needed), open the app, and look for the new **View Route Map** button on the home screen. Please let me know plainly whether it even builds — that's the real open question here, more than usual.

...
**Last updated:** 28 July 2026 · **Phase:** C — approved. **Stage 2 (A, B, C) — ALL COMPLETE. Stage 3 (3.1–3.4) — ALL APPROVED. Full database security hardening — DONE, verified. Stage 4 (4.1, 4.2, 4.3) — ALL APPROVED. S5.1 — APPROVED. S5.2 — APPROVED. S5.3 — APPROVED. S5.4 — CONFIRMED WORKING on founder's machine — this completed the entire planned MVP build sequence. Post-MVP backlog: PDF/PowerPoint report exports — CONFIRMED WORKING. Configurable dashboard engine for clients — CONFIRMED WORKING. Offline map corridors — CONFIRMED WORKING on founder's own physical phone. Perceptual duplicate detection — CONFIRMED WORKING, founder viewed the flag live on the Device Risk page. Regional languages (EN/HI) — CONFIRMED WORKING on founder's own physical phone, including two navigation/session bugs found during that testing and fixed. AI image analysis — CONFIRMED WORKING on founder's own physical phone (quality-warning flow) and in the browser (Media Review page). This completes the entire Post-MVP backlog as originally scoped. Also this session (not an original backlog item, a founder usage request): PJP location suggestions (State/District canonical + campaign-history Tehsil/Location) and an interactive OpenStreetMap picker on the "Add a location manually" form — built and verified via API/build checks, awaiting founder browser review.**

## Post-MVP backlog — AI image analysis (this session, founder picked this as the final backlog item)

Spec §42 explicitly deferred this from the MVP ("keep the architecture ready for them") without saying what it should actually check — confirmed scope with the founder first: real photo-quality checks (blur, exposure, resolution) plus a clearly-labelled mocked "smart content" classifier adapter (same MOCK pattern as OTP/WhatsApp), warn the field worker immediately on a bad photo AND flag it for supervisors, and a new dedicated "Media Review" screen (a required screen from spec §40 that had never been built).

- **The quality checks are real, not a placeholder** — a well-established technique (variance-of-Laplacian for blur, mean brightness for exposure) computed twice: once on the phone right after the photo is taken (so the field worker can retake it on the spot), and again on the server when the photo finishes uploading (so a supervisor sees the same flag later). Calibrated against real generated test images on both sides, not guessed numbers.
- **The "smart content" classifier is honestly labelled MOCK** — there's no real AI vision service connected in this environment (that would need a paid cloud provider and real credentials, which this project never puts in code), so this part of the architecture exists and is wired up, ready for a real provider later, but doesn't pretend to classify anything yet. Every place it shows up says so plainly.
- **New "Media Review" page** for supervisors — browse evidence photos for a campaign, filter to quality-flagged ones only, same photo-with-watermark viewing already used elsewhere.
- **Verified for real**: uploaded an in-focus photo, a genuinely blurred one, and a genuinely darkened one through the actual phone-app upload path — the blurred one was correctly flagged, the dark one was correctly flagged, the in-focus one correctly wasn't. Checked the new Media Review data as a real supervisor account too.
- **CONFIRMED by the founder** on their own physical phone (took a deliberately bad photo, saw the retake-or-use-anyway warning) and in the browser (opened Media Review, saw the photo with its quality flag and the "Content: MOCK" badge, tried the flagged-only filter) — 28 July 2026. A fresh test assignment ("AI Image Analysis Test Stop") had to be created first since the logged-in test account had no assignment for today.

### Reviewing — AI image analysis

CONFIRMED. Nothing pending here.
...
Per spec §17/§18 and the architecture docs' own note that this was deliberately deferred past the MVP ("exact-hash duplicates in MVP; perceptual similarity post-MVP") — catching a field worker re-using an old photo, or a re-compressed/lightly-cropped copy of one, as if it were fresh evidence from today's visit.

- **Plugs into the Device Risk system you already know about (S4.2)**, rather than being a separate new feature — a suspiciously-similar photo now raises the exact same kind of flag as a mismatched device clock or a mock-location signal, showing up in both the Device Risk page and the general Alerts inbox.
- **The detection itself is a real, standard image-comparison technique** (not an invented shortcut) — every evidence photo gets a small "fingerprint" computed from its visual content, and a new photo's fingerprint is compared against that same field worker's last 200 or so photos in the campaign. A re-compressed or lightly-cropped copy of an old photo produces a near-identical fingerprint even though the file itself is completely different bytes; a genuinely different photo doesn't.
- **Tested for real, both ways** — uploaded an original photo, a re-compressed near-duplicate of it, and a genuinely different photo through the real upload flow. The near-duplicate correctly raised a flag; the genuinely different one correctly did not (checked for false alarms, not just the success case).
- **Verified**: backend and web build clean; full test suite still passing; QA grep clean.

### Reviewing — Perceptual duplicate detection

There's nothing new to look at directly — this works silently in the background the same way device-risk detection already does. If you want to see it fire, ask and a test photo pair can be uploaded for you to review on the Device Risk page.

## Post-MVP backlog — Offline map corridors (this session, founder picked this after the configurable dashboard engine)

Per spec §20 ("Offline maps") — the phone downloads a small map of just the area it needs (district/tehsil/route/assigned stops) so the field team can see a real map with zero signal, not just a blank screen.

- **Founder asked for this to actually be tested, not just written and hoped for — so the full Android toolchain (Flutter, Android SDK, a compatible Java) was installed on this build machine and a real Android emulator was set up and booted.** This is the first time this project has ever had a working emulator available. The whole feature was then driven for real: installed the app, logged in as Rahul Kumar, tapped through to the new map screen, granted the real location permission prompt, and watched it render.
- **Two real bugs were found this way and fixed before calling it done** — not just a clean compile, an actual working feature:
  - The map first showed as blank grey tiles. The real cause: the phone's actual screen needs a wider strip of map tiles than the corridor was fetching. Fixed, and re-confirmed with a real, correct, readable street map of Patna (Gandhi Maidan, Patna Junction, Bailey Road all visible).
  - Turning the emulator's WiFi and mobile data off and reopening the map screen then failed outright, even though the map images were already saved — because the day's stop list itself was being re-fetched over the network every time, with no saved copy. Fixed, and re-confirmed: with the network fully switched off, the map still opened correctly from what was saved earlier — the actual point of "offline map corridors."
- **Verified, for real this time**: builds clean; installed and ran on a real (emulated) Android device; full login → home → route map flow worked; the map rendered a genuine, correct, readable street map with the right marker in the right place; confirmed working with the network fully disabled. Not yet tested: a physical phone, or a route with more than one stop.

### Reviewing — Offline map corridors

1. Restart with `./scripts/bootstrap.sh` if needed. No new setup required — this uses your existing database and file storage.
2. On your phone (Android Studio or `flutter run`, same as always): log in as Rahul Kumar (`9000000009`) with a real assignment for today that has a location (see "How to see it yourself" below for creating a test assignment if needed), and tap the new **View Route Map** button on the home screen.
3. Once it's loaded once with signal, try turning on airplane mode and opening it again — it should still show the map.
...
35. **Offline map corridors — backend built and verified working; phone-app half built but not yet checked at all, 24 July 2026.** The server side that fetches and saves map images for a field worker's route was tested for real with genuine coordinates in Patna and produced a correct, real map. The phone-app screen to show that map is written but — unlike every prior phone-app change in this project — this build machine had no Flutter toolkit installed at all this time, so not even the basic "does this have typos" check could run, let alone a real test on a phone. Please treat this one with extra caution until you (or a session with Flutter available) can confirm it actually builds. See "Reviewing — Offline map corridors" above.
36. **Offline map corridors — CONFIRMED WORKING end-to-end on a real Android emulator, 24 July 2026** ("Lets test this, please install all the app needs in the system for testing, install flutter"). Installed the full Android/Flutter toolchain on this build machine, booted a real Android emulator for the first time in this project, and drove the whole feature for real: login, home screen, the new Route Map button, granting the real location permission prompt, and watching it render. Two genuine bugs were found this way and fixed before calling it done — the map first rendered as blank grey (the corridor wasn't fetching quite enough tiles to cover a real phone screen) and then, once that was fixed, reopening the map with the network fully switched off failed outright (the day's stop list was being re-fetched every time with no saved copy, even though the map images themselves were already saved). Both fixed and re-confirmed — the second one specifically re-tested with WiFi and mobile data both off, and the map still opened correctly from what was saved earlier. This is now genuinely verified, not just written and hoped for. See docs/ASSUMPTIONS.md A-076 for the full detail. Not yet tested: a physical phone, or a route with more than one stop.
37. **Offline map corridors — CONFIRMED WORKING on your own physical phone, plus two more pieces you asked for on the spot, 24 July 2026.** Connected your phone by USB and got the same map working there — one quick, expected snag (a new device needing approval, cleared the same way as always) and then it worked. You then asked two good questions that led to two real additions: **distance to your next stop** (a banner at the top of the map, plus tapping any stop shows its distance from you — this was already in the spec's own list of what this screen should do, not built yet until now) and **"Open in Google Maps"** on that same tap-to-see-details popup, for when you do have signal and want turn-by-turn directions (the spec itself says external navigation apps should be offered "when available" — this is exactly that). Both confirmed working by you directly on your phone. This closes out the third Post-MVP backlog item, genuinely device-tested this time. Remaining backlog: perceptual duplicate detection, regional languages, AI image analysis, and the Impact IQ predictive side (built separately by Impact).
38. **Perceptual duplicate detection — built and verified against a real running backend, 27 July 2026.** Catches a field worker re-using an old photo (or a re-compressed/cropped copy of one) as fresh evidence — this was always meant to come after the MVP, per the architecture docs' own note that exact-duplicate detection (already working) would ship first and this "near-duplicate" version later. Plugs straight into the Device Risk system you already know — a suspicious photo raises the same kind of flag as a mismatched clock or a mock-location signal. Tested for real both ways: a genuine near-duplicate correctly raised a flag, and a genuinely different photo correctly did not. This closes out the fourth Post-MVP backlog item. Remaining: regional languages, AI image analysis.
